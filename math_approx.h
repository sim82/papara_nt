...
#ifndef MATH_APPROX_H_
#define MATH_APPROX_H_
...
public:
    ...
    log_func( size_t precision = 12 )
    : p_table_( ::pow(2,precision) ),
      shift_bits_( 12 + 20 - precision )
    {
        /*
        ...
        assert( p_table_.size() == (1 << precision) );

        for(i = 0;  i < (1 << precision);  ++i )
        {
            // make y-axis value for table element
            p_table_[i] = ::log(oneToTwo);
    ...
    inline double operator()(double input)
    {
        ...
        else
        {
            ...
            {
                if( check_ && input > std::numeric_limits<double>::max() )
                    result = std::numeric_limits<double>::infinity();
                else
                {
                    ...
                    register int index = (left << 12) >> shift_bits_;

                    result = (con_value_ * m1  + p_table_[index]);
                }
            }
        }
        return result;
...
private:
    const static double con_value_;

    std::vector<double> p_table_;
    const size_t    shift_bits_;
    const static bool check_ = false;
};


const double log_func::con_value_ = ::log(2);
...
float log_sse( float v ) {
    float ALIGN16_BEG res[4] ALIGN16_END;

    _mm_store1_ps(res, log_ps(_mm_set1_ps(v)));

    return res[0];
}

float exp_sse( float v ) {
    float ALIGN16_BEG res[4] ALIGN16_END;

    _mm_store1_ps( res, exp_ps(_mm_set1_ps(v)));
    return res[0];
}
