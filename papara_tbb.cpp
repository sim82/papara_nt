...
struct block_result {
    ...
    std::vector<std::array<scalar,VW>> scores_;
    
    
};

template<typename seq_tag>
block_result<seq_tag> execute_block( const typename block_queue<seq_tag>::block_t &block, const queries<seq_tag> &qs_, const papara_score_parameters &sp_ ) {
    typedef typename vu_config<seq_tag>::scalar vu_scalar_t;
    typedef typename block_queue<seq_tag>::block_t block_t;
    typedef model<seq_tag> seq_model;
    
    const size_t VW = block_queue<seq_tag>::VW;
    ...
    block_result<seq_tag> res;
    res.block_ = &block;
    res.scores_.resize( qs_.size() );
    ...
    for( unsigned int i = 0; i < qs_.size(); i++ ) {
        ...
        // std::array's lack of useful constructor/assign is understandable but annoying...
        std::copy( out_scores.begin(), out_scores.end(), res.scores_[i].begin() );
...
//     const size_t VW = block_queue<seq_tag>::VW;
    ...
    auto first = bq.raw_blockqueue().begin();
    auto last = bq.raw_blockqueue().end();
    ivy_mike::timer t1;
    tbb::parallel_pipeline( /*max_number_of_live_token=*/16,       
        tbb::make_filter<void,const block_t *>(
            tbb::filter::serial,
            [&](tbb::flow_control& fc)-> const block_t * {
                std::cout << "fc: " << std::distance( first, last ) << "\n";
                if( first != last ) {
                    return &(*first++);
                } else {
                    fc.stop();
        ...
        tbb::make_filter<const block_t*, block_result<seq_tag> >(
            tbb::filter::parallel,
            [&qs, &sp](const block_t* block){
                block_result<seq_tag> res;
                
                assert( block != nullptr );
                return execute_block<seq_tag>( *block, qs, sp );
            } 
        ) &
        tbb::make_filter<block_result<seq_tag>,void>(
            tbb::filter::serial,
            [&](block_result<seq_tag> my_res) {
                const auto &block = *(my_res.block_);
                
                std::cout << "gather\n";
                for( size_t i = 0; i < my_res.scores_.size(); ++i ) {
                    res->offer_nolock( i, block.edges, block.edges + block.num_valid, my_res.scores_[i].begin() );
