         ...
         if (m > l) {
            ...
            do {
               ...
               double p = (d(l+1) - g) / (2.0 * e(l));
                double r = boost::math::hypot(p,1.0);
               if (p < 0) {
                  r = -r;
               }
               d(l) = e(l) / (p + r);
...
static inline void cdiv(double xr, double xi, double yr, double yi, double &cdivr, double &cdivi) {
      double r,d;
...
EigenvalueDecomposition::EigenvalueDecomposition (const Matrix& A) {
	  ...
	  assert( A.size2() < std::numeric_limits<int>::max() );
...
EigenvalueDecomposition::Matrix EigenvalueDecomposition::getD () const {
      Matrix D(n,n);
      for (int i = 0; i < n; i++) {
         for (int j = 0; j < n; j++) {
            D(i,j) = 0.0;
